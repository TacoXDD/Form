        ...
        if(["address", "combined", "datetime", "time"].include(type)) {
            ...
            if(type == "checkbox" || type == "radio") {
                ...
                if($('input_' + field).triggerEvent) {
                    $('input_' + field).triggerEvent(type == "widget" ? 'change' : 'keyup');
                }
                ...
                switch(fieldType){
                    case "combined":
                        if (['isEmpty', 'isFilled'].include(term.operator)) {
                            ...
                            //for matrices
                            value = $$('#id_'+term.field+' input').collect(function(e){ return e.value; });
                    ...
                    case "select":
                        ...
                        if($('input_'+term.field) && $('input_'+term.field).multiple) {
                            ...
                            filled = $$('#id_'+term.field+' select').collect(function(e){ return e.value; }).any();
                            if(JotForm.checkValueByOperator(term.operator, term.value, filled)){
                                any = true;
                            }else{
                                all = false;
    ...
    validator: function(){
        ...
        $A(JotForm.forms).each(function(form){ // for each JotForm form on the page 
            ...
            form.observe('submit', function(e){ // Set on submit validation
                ...
                $$('.form-checkbox, .form-radio').each(function(el) { el.enable(); });
